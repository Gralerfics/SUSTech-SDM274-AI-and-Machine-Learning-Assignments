...
def eval_multiclass_one_hot_accuracy(Y: Union[Variable, np.ndarray], T: np.ndarray):
    """
...
def eval_multiclass_label_accuracy(Y: Union[Variable, np.ndarray], T: np.ndarray):
    """
        Y: the output of the model (one of the integers)
        T: the target (label) of the model output (one of the integers)
    """
    if isinstance(Y, Variable):
        Y = Y.value
    if Y.ndim == 2:
